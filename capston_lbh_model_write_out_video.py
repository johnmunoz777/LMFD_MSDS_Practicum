...
def process_video(input_video_path, output_video_path,
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            # ✅ FIX: Set bounding box color dynamically
            rect_color = (0, 255, 0) if detected_status.lower() == "active" else (0, 0, 255)

            # Draw bounding box with correct color
            ...
            if profile is not None:
                ...
                for i, field_name in enumerate(FIELD_NAMES):
                    if i < len(profile):
                        ...
                        cv2.rectangle(frame, 
                                      (x, startY + i * LINE_HEIGHT - text_h - 5), 
                                      (x + text_w, startY + i * LINE_HEIGHT + 5), 
                                      (0, 0, 0), cv2.FILLED)
                        cv2.putText(frame, text, 
                                    (x, startY + i * LINE_HEIGHT), 
        ...
        if highest_conf_info is not None:
            ...
            global_text = label_output_model if label_output_model else ""
...
process_video(iv, "output_video_ron.mp4", rotate_mode=1, 
              label_output_model="Face Recognition Model Output", 
              adjust=0.8, resize_video=(1280, 1280), conf_threshold=50)
